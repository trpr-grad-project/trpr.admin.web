...
function getStatusBadgeStyles(status: string): string {
  switch (status) {
    case "Approved": return "bg-success-container/20 text-success border-success-container/50";
    case "Rejected": return "bg-error-container text-error border-error-container/30";
    default: return "bg-tertiary/10 text-secondary border-tertiary-container/30";
  }
}
    ...
    <section className="flex-1 relative">
      <header className="flex justify-between items-end">
        ...
        <div className="flex gap-4">
          <span className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-2 border ${getStatusBadgeStyles(request.status)}`}>
            <span className={`w-2 h-2 rounded-full animate-pulse ${getDotStyles(request.status)}`}></span>
            {request.status}
          </span>
        </div>
      </header>
